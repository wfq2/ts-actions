...
export const invalidStep = (step: Step<string | null>) =>
  step.name("Invalid step").uses(ActionsSetupNode4).with({ "node-version": "24" }).run("npm ci"); // This should cause an error
